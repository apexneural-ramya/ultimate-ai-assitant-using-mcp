...
def create_agent(client):
    """Create MCPAgent with the client, using OpenRouter."""
    try:
        api_key = os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY")
        if not api_key:
            st.error("Missing OPENROUTER_API_KEY (or OPENAI_API_KEY) in environment.")
            return None

        # Use a valid OpenRouter model ID. "openai/gpt-4o-mini" is supported.
        llm = ChatOpenAI(
...
with st.sidebar:
  ...
  "mcpServers": {
    ...
    "ragie": {
      ...
      "env": {
        "RAGIE_API_KEY": "your-ragie-key-here"
      }
    }
  }
}''',
    ...
    # Load example configuration (Firecrawl + Ragie only)
